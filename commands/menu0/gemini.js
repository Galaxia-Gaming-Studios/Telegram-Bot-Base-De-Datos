...
module.exports = (bot) => {
    bot.command('gemini', async (ctx) => {
        try {
            ...
            if (args.length === 0) {
                ctx.reply('🏮 Tienes que hacer una pregunta después del comando. Ejemplo: /gemini ¿Eres mujer?');
                return;
            }

            let userPregunta = args.join(' '); // Recoge la pregunta del usuario
            const url = `https://deliriusapi-official.vercel.app/ia/gemini?query=${encodeURIComponent(userPregunta)}`;
...
[❦┕ᴡᴀɪғᴜ ⭐ʙᴏᴛ⊡❥](https://chat.whatsapp.com/BcTvQlXoNBLJPMJjbtdElS)`;
                ...
                ctx.reply('🍫 Gemini no pudo procesar su mensaje.');
            }
